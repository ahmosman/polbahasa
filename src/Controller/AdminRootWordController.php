...
class AdminRootWordController extends AbstractController
{
    ...
    {
        return new Response($this->twig->render('admin_root_word/show.html.twig', [
            'rootWords' => $rootWordRepository->findAllOrderBy('name','asc')]
        ));
    }
    ...
    public function add(Request $request)
    {
        ...
        if($name = $request->query->get('name'))
        {
            ...
            $this->em->flush();
            $this->addFlash('success', 'Pomyślnie dodano podstawę słowotwórczą');
            ...
            return new Response($this->twig->render('admin_root_word/add.html.twig', [
    ...
    public function edit(Request $request, RootWord $rootWord)
    {
        ...
        if ($form->isSubmitted() && $form->isValid())
        {
            $this->em->persist($rootWord);
            $this->em->flush();
            $this->addFlash('success', 'Pomyślnie zmieniono podstawę słowotwórczą');
            return $this->redirectToRoute('admin_root_word_show');
        }
        return new Response($this->twig->render('admin_root_word/add.html.twig', [
            'form' => $form->createView()
    ...
    public function checkexist(RootWordRepository $rootWordRepository)
    {
        ...
        $rootWord = $rootWordRepository->findOneBy(['name' => $rootWordName]) ?? null;
